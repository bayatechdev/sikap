...
function SplitHeroSection({ title, subtitle, primaryButton, secondaryButton, images }: SplitHeroProps) {
  const leftVariants = {
...
// Full Slider Hero Section Component
interface FullSliderHeroProps {
  title: string;
  subtitle: string;
  primaryButton: string;
  secondaryButton: string;
...
function FullSliderHeroSection({ title, subtitle, primaryButton, secondaryButton, images }: FullSliderHeroProps) {
  return (
    <HeroFullSlider
      images={images}
      autoPlayInterval={6000}
...
export default function HeroSection({ data }: HeroSectionProps) {
  ...
  const renderHeroSlides = () => {
    ...
    return heroSlides.map((slide) => {
      const slideTitle = slide.title || globalHeroTitle;
      const slideSubtitle = slide.subtitle || globalHeroSubtitle;
      const slideImages = (slide.imagesJson || []).map(img => ({
        url: img.url,
        alt: img.alt || slideTitle,
        title: img.title
      }));

      if (slide.version === 'split') {
        return (
          <SplitHeroSection
            key={slide.id}
            title={slideTitle}
      ...
      } else if (slide.version === 'full') {
        return (
          <FullSliderHeroSection
            key={slide.id}
            title={slideTitle}
            subtitle={slideSubtitle}
            primaryButton={primaryButton}
